...
Towns_Population = read_csv("Clean/Cleaned_Town_population.csv") %>% 
  select(-Year) 
HousePrices=read_csv("Clean/Cleaned_House_Pricing_2019-2022.csv", show_col_types = FALSE)
...
write.csv(House_town,"Clean/HOUSE_TOWN.csv",row.names = FALSE)
...
House_town %>% 
  ...
  geom_boxplot() +
  coord_flip() +
  labs(title="2019-2022 house prices by county")



# BARGRAPH house prices of both county (2020-2022)
HousePricesclean %>% 
  filter(Year == 2019  | Year == 2021 |Year==2022) %>% 
  group_by(County) %>% 
  summarise(AveragePrice = mean(Price)) %>% 
...
Town = read_csv("Clean/Cleaned_Town_population.csv")%>% 
  select(-Year)
crime_Data = read_csv("Clean/Combined_Crime_Data.csv")
...
schoolData %>%
  ...
  geom_boxplot(color = "purple", fill = "blue") +  
...
color= c("OXFORDSHIRE" = "brown", "YORKSHIRE" = "black")
...
color= c("OXFORDSHIRE" = "blue", "YORKSHIRE" = "Green")
...
# Create a scatter plot
color <- c("OXFORDSHIRE" = "black", "YORKSHIRE" = "Green")
